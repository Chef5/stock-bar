...
// 如果远程开发时插件无法使用，可以强制Stock Bar扩展在本地UI端运行
...
当配置了 `hold_number`（大于0）时，插件会自动：

1. 在状态栏股票名称后显示 **当日盈亏**，如：`平安 12.50 +1.25% [+250]`
2. 在悬浮提示中显示 **当日盈亏** 和 **持仓盈亏**
3. 在状态栏末尾显示 **账户当日盈亏汇总**（`今日：+xxx`），汇总所有持仓股票的当日盈亏，悬浮可查看各股票明细

如需关闭上述盈亏信息展示，可将 `"stock-bar.showAccountPnL"` 配置为 `false`。

> 当日盈亏 = 涨跌额 × 持仓数量，持仓盈亏 = (现价 - 持仓价) × 持仓数量
